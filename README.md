...
## 10. Reproducibility & ICML-style Report

### 10.1 Reproducibility Recommendations

* Unify global random seeds, e.g., `SEED = 2025`.
* Fix environment versions (Python, PyTorch, Transformers, Opacus, etc.).
* All training scripts write configuration JSON and logs in `out_dir`, and append a CSV record in `results/`.
* Use `scan_phi_like.py` to perform sanity checks on B-domain data and model outputs, ensuring no accidental leakage.

### 10.2 Correspondence with ICML Report Sections

* **Introduction**
  Medical NLI + domain shift + privacy risk background; propose RQ1–RQ4 and contributions.

* **Problem Setup & Threat Model**
  Define A/B domain settings; describe attacker capabilities and access interfaces (A/B domain queries, black-box/white-box).

* **Data Construction (Phase I)**
  Explain A-domain standardization & synthetic PHI; explain the strategy for generating B domain using `hospitalB_shift_pipeline.py`.

* **Methods (Phase II & III)**

  * BERT + classification head + DP-SGD (DP-BERT);
  * DANN and DP-DANN architecture, loss functions, and training strategies.

* **Experiments**

  * Utility: B-test performance, A→B gap, whether DANN improves cross-domain performance;
  * Privacy: MIA, PHI-like scanning, cross-domain leakage analysis;
  * Utility–Privacy trade-off curves and tables.

* **Discussion**

  * Impact of DP on domain adaptation (whether it weakens DANN effects, whether it changes representations);
  * The "attack A, observe B" phenomenon and mechanism;
  * Limitations of synthetic PHI and implications for real PHI.

* **Conclusion & Future Work**
  Summarize main findings of DP-MedNLI; prospects for stronger models (larger BERT / LLM), stronger attackers, and real PHI settings.
